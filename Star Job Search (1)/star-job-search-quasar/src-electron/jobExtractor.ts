...
export function createJobExtractor(deps: JobExtractorDeps): {
  ...
  async function discoverNode(state: ExtractorState): Promise<Partial<ExtractorState>> {
    ...
    const result = await structured.invoke(
      `Inspect this job-board listing page (host=${state.hostname}).
...
${(sample ?? '').slice(0, 12000)}`,
    ...
    return { selectors: { ...result, nextSelector: result.nextSelector ?? null } };
  }
  ...
  function routeAfterEnumerate(state: ExtractorState): 'paginate' | 'dedup' {
    const next = state.selectors?.nextSelector;
...
${(body ?? '').slice(0, 16000)}`,
  ...
  const graph = new StateGraph<ExtractorState>({
    ...
    .addConditionalEdges(
      ...
      { paginate: 'paginate', dedup: 'dedup' } as never,
    )
